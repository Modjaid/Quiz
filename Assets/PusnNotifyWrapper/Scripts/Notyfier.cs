...
namespace NotificationSamples
{
	...
	/// </summary>
	public class Notyfier : MonoBehaviour
	{

		public static Notyfier notifier;
...
#pragma warning restore CS0649
		...
		private void Initialize(params GameNotificationChannel[] channels)
		{
			if (Initialized)
			{
		...
		private PendingNotification ScheduleNotification(IGameNotification notification)
		{
			if (!Initialized)
			{
				throw new InvalidOperationException("Must call Initialize() first.");
		...
		public void Awake()
        {
			try
			{
				notifier = new Notyfier();
				GameNotificationChannel channel = new GameNotificationChannel("defaultTitle", "defaultBody", "Здесь должно быть описание");
				notifier.Initialize(channel);
			}catch(ArgumentNullException ex)
            {
				//Debug.Log("Пуш уведомлялка будет работать только при билде");
            }

		}

		public static void sendMessageToPush(string titleText, string bodyText, int deliveryTime)
		{
			IGameNotification notification = notifier.CreateNotification();
			if (notification != null)
			{
				notification.Title = titleText;
				notification.Body = bodyText;
			//	notification.SmallIcon = "icon_0";
				notification.DeliveryTime = DateTime.Now.AddSeconds(deliveryTime);
				//Message = "Пуш придет через " + (deliveryTime <= 5 ? 5 : deliveryTime) + " сек";
				notifier.ScheduleNotification(notification);
			}
		}
	}
}